...
c     Gather all the data from the input file
     ...
     $   h, target_density, density_tol, mu, uu,
     $   up, uj, ed, tij,
...
c Put temperature in eV
      t = kb*t
c Multiply H by mu_B to get eV
      h = mub*h
      h_pert = mub*h_pert 
...
#ifdef SECOND_ORDER
      iteration = 0
      call effective_field(iteration, 
     $   v_pert, h, h_pert, prfld, 
     $   prfld_pert, v_pert_eff, h_eff, prfld_eff)
      call discontinuities(ek, v_pert_eff, psi, h_eff, 	
...
c     Prepare for self-consistency loop.
...
#ifdef SECOND_ORDER
          call discon_lat(ek, v_pert_eff, psi, h_eff, 	
     $       prfld_eff, mu, sigma1, h_so, 
     ...
     $       delta_gp_r, delta_gp_k)
	  ...
	  call dyson(rank, g, g_tau0, q_tau, q_epsilon, 
...
#else
          call calc_g_tau0(ek, v_pert_eff, psi, h_eff, 	
     $       prfld_eff, mu, sigma1, h_so, 
...
C$$$#endif /* THIRD_ORDER */
     ...
     $       mub*(gs/2.0d0)*h(ib,1)*magb(ib,1)
        enddo
        write(6,*) 'Mx_spin per atom/((g/2)*mub) = ', mag,
     $     convergence_text
     ...
     $       g_tau0_local(4*ib+0,4*ib+1)) ) 
          mag = mag + magb(ib,2)
          mag_energy = mag_energy - 
     $       mub*(gs/2.0d0)*h(ib,2)*magb(ib,2)
        enddo
        write(6,*) 'My_spin per atom/((g/2)*mub) ', mag,
     $     convergence_text
     ...
     $       mub*(gs/2.0d0)*h(ib,3)*magb(ib,3)
        enddo
        write(6,*) 'Mz_spin per atom/((g/2)*mub) ', mag,
     $       convergence_text
     ...
     $       magb(ib,3)
        ...
        do ib = 0, nb-1
          do jb = 0, nb-1
            ...
            if (ib .eq. jb) then            
              Lorb = Lorb + Lvec(:,ib,ib) * 
     $           (g_tau0_local(4*ib,4*ib) +
     $           g_tau0_local(4*ib+1,4*ib+1) + 2.0d0)
            else
              Lorb = Lorb + Lvec(:,ib,jb) * 
     $           (g_tau0_local(4*jb,4*ib) +
 ...
 225    format(i3,2x,d16.8,2x,d16.8,2x,d16.8,2x,d16.8)
         ...
         do is = 0, 1
          do jb = 0, nb-1
            do js = 0, 1
              ...
              so_energy = so_energy + h_so(2*ib+is,2*jb+js)*
     $             glocal
        ...
        write(6,*) 'Spin-orbit energy = ', real(so_energy),
     $    convergence_text
        write(6,*) 'Potential energy = ', pot_energy,
     $    convergence_text
        write(6,*) 'Note: magnetic energy neglects orbital terms'
        write(6,*) 'Magnetic energy = ', mag_energy,
     $     convergence_text 
        call hfock_energy(sigma1, g_tau0_local, e_hf)
        write(6,*) 'Hartree-fock energy = ', e_hf,
     $     convergence_text
      endif
     ...
     $   c_r, d_r, q_tau, q_mtau, r_tau, x, y)
      ...
      if (rank .eq. 0) then
        write(6,*) 'Correlation energy = ', cor_energy, 
     $ convergence_text
      endif
#endif /* SECOND_ORDER */
      if (rank .eq. 0) then
