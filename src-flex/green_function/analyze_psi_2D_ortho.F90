...
c     extended s, y^2
      ...
      do ib = 0, nb-1
        do jb = 0, nb-1
          dot = psi(2*ib+0,2*jb+1,llx) - 
     $       psi(2*ib+1,2*jb+0,llx) 
          dot_real = real(dot)
          dot_imag = imag(dot)
          mag = sqrt( dot_real**2 + dot_imag**2)
          write(6,200) ib, jb, mag, atan2(dot_real, dot_imag) / pi      
...
c     extended s, 2y^2
      ...
      do ib = 0, nb-1
        do jb = 0, nb-1
          dot = psi(2*ib+0,2*jb+1,2*llx) - 
     $       psi(2*ib+1,2*jb+0,2*llx)
          dot_real = real(dot)
          dot_imag = imag(dot)
          mag = sqrt( dot_real**2 + dot_imag**2)
          write(6,200) ib, jb, mag, atan2(dot_real, dot_imag) / pi      
...
c     extended s, x+y (A2)
      ...
      do ib = 0, nb-1
        do jb = 0, nb-1
          dot = psi(2*ib+0,2*jb+1,llx+1) - 
     $       psi(2*ib+1,2*jb+0,llx+1) 
          dot_real = real(dot)
          dot_imag = imag(dot)
          mag = sqrt( dot_real**2 + dot_imag**2)
          write(6,200) ib, jb, mag, atan2(dot_real, dot_imag) / pi      
...
c     extended s, x-y (A2)
      ...
      do ib = 0, nb-1
        do jb = 0, nb-1
          dot = psi(2*ib+0,2*jb+1,llx+llx1) - 
     $     psi(2*ib+1,2*jb+0,llx+llx1) 
          dot_real = real(dot)
          dot_imag = imag(dot)
          mag = sqrt( dot_real**2 + dot_imag**2)
          write(6,200) ib, jb, mag, atan2(dot_real, dot_imag) / pi      
...
c     extended s, 2x+2y (A2)
      write(6,*) "**** extended s, 2x+2y (A2) ****"
      write(6,*) "ib  jb amp      phase/pi"
      do ib = 0, nb-1
        do jb = 0, nb-1
          dot = psi(2*ib+0,2*jb+1,2*llx+2) - 
     $       psi(2*ib+1,2*jb+0,2*llx+2) 
          dot_real = real(dot)
          dot_imag = imag(dot)
          mag = sqrt( dot_real**2 + dot_imag**2)
          write(6,200) ib, jb, mag, atan2(dot_real, dot_imag) / pi      
