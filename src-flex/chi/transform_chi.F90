...
c **************************************************************/
      ...
      subroutine transform_chi(rank, t, chi, delta_chi, 
...
#include "../constants.F90"
     ...
     $    delta_chi, delta_chi_prime
      ...
      COMPLEX, dimension (0:16*nb*nb-1, 0:16*nb*nb-1) ::
     $   temp_matrix, vl, vr
...
c     Determine the dominant eigenvector of chi and
c     artificially rescale singular chi(w,k) matrices.
      jobvl = 'N'
      jobvr = 'V' 
     ...
     $       16*nb*nb, work, 512*nb*nb, rwork, info)
          ...
          do i = 0, 16*nb*nb-1
            ...
            if (real(eigenvalue(i)) .gt. 
     $         this_eigenvalue_max) then
                  
              this_eigenvalue_max =
     $           real(eigenvalue(i))

              if (real(eigenvalue(i)) .gt.
     $           overall_eigenvalue_max) then

                overall_eigenvalue_max = real(eigenvalue(i))
                do j = 0, 16*nb*nb-1
                  dominant_chi_eigenvector(j) =
     $               vr(j,i)
                enddo
     ...
     $       MPI_COMPLEX, 
     $       i, 3, MPI_COMM_WORLD, stat, ierr)
          call MPI_Send(dummy, 1, MPI_INTEGER, i, 4,
     $       MPI_COMM_WORLD, ierr)  
          call MPI_Recv(tmp_index, 2, MPI_INTEGER, i, 5,
     $        MPI_COMM_WORLD, stat, ierr)

          if (tmp_eigenvalue_max .gt. 
     $       overall_eigenvalue_max) then
     ...
     $           tmp_eigenvector(j)
            ...
            dominant_chi_index(0) = tmp_index(0) +
     $         mp * i
