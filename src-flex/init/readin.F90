#include "../convert.F"
     ...
     $   ed, tij, prfld_pert, h_pert, v_pert, h_so,
...
c     other variable
     ...
     $   MPI_COMM_WORLD, ierr)
      call MPI_Bcast(h, 3*nb, MPI_REAL, 0,
     $   MPI_COMM_WORLD, ierr)
...
c-----Perturbations: applied and relaxed over initial 10 iterations -------
     ...
     $   " initial 10 iterations"
        ...
        write(6,*) "Random orb.-dependent mag. field amp (T) ", 
     $     h_pert_amp
        do ib = 0, nb-1
        ...
        enddo
        write(6,*) "Random orbital potential amplitude  = ",
     $     v_pert_amp
        do ib = 0, nb-1
     ...
     $   MPI_COMM_WORLD, ierr)
      call MPI_Bcast(h_pert, 3*nb, MPI_REAL, 0,
     $   MPI_COMM_WORLD, ierr)
      call MPI_Bcast(v_pert, nb, MPI_REAL, 0,
     $   MPI_COMM_WORLD, ierr)
#endif
        ...
        if (so_flag) then
          ...
          do ibs = 0, 2*nb-1
            do ibsp = 0, 2*nb-1
              ...
              write(6,300) ibs, ibsp, real(h_so(ibs,ibsp)),
     $           imag(h_so(ibs,ibsp))
            enddo
          enddo
          h_so = so_amp*h_so
...
#ifdef USE_MPI
      call MPI_Bcast(h_so,4*nb*nb, MPI_COMPLEX, 0,
     $   MPI_COMM_WORLD, ierr)
#endif
