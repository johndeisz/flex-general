...
c     Add the sum back to the transformed T arrays
          do l = 0, mp1
            do k = 0, nc1
     ...
     $               d(ia,ib,i,j,k)
              ...
              temp_4D(l,k) = 
     $           t * temp_4D(l,k) + sum_dr

            enddo
          enddo
  
c     Transform on the space-indices
          ifftv(0) = 0
          ifftv(1) = 1
          ifftv(2) = 1
          ifftv(3) = 1
...
c     Transform delta_t and delta_t_prime
      ifftv(0) = 0
      ifftv(1) = 1
      ifftv(2) = 1
      ifftv(3) = 1
